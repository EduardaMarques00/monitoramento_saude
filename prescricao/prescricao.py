...
def buscar(id_prescricao):
    with engine.connect() as conn:
        ...
        row = result.fetchone()
        if row is None:
            return jsonify({"ok": False, "erro": "Prescrição não encontrada."}), 404
...
def inserir():
    ...
    try:
        ...
        return jsonify({"ok": False, "erro": "Erro de integridade ao cadastrar prescrição.", "detalhe": str(e.orig)}), 409
...
def atualizar(id_prescricao):
    ...
    try:
        ...
        return jsonify({"ok": False, "erro": "Erro de integridade ao atualizar prescrição.", "detalhe": str(e.orig)}), 409
...
def remover(id_prescricao):
    try:
        ...
        return jsonify({"ok": False, "erro": "Não é possível remover esta prescrição.", "detalhe": str(e.orig)}), 409
